...
bot.onText(/\/start/, async msg => {
   ...
   const foundCards = await model.foundCards(chatId)?.count

   if (foundUser) {

      if (foundCards == 0) {
         bot.sendMessage(chatId, localText.startText1)
         bot.sendMessage(chatId, localText.askCardNumberStart)
            .then(async () => {
...
bot.on('message', async (msg) => {
   ...
   if (text == localText.uz) {
      ...
      if (foundUser?.lang == 'uz') {
         ...
         const formatText = localText?.askContactCyrillic.replace(/%name%/g, text)
         bot.sendMessage(chatId, formatText, {
            reply_markup: {
               keyboard: [
                  [{
      ...
      const verifyNumber = hasExact16Numbers(text?.split(' '))
      ...
      if (foundUser?.lang == 'uz') {
         ...
         if (verifyNumber) {
            ...
            if (addCardNumber) {
               bot.sendMessage(chatId, localText.askExpiredUz)
                  .then(async () => {
                     await model.editStep(chatId, 'ask_expired')
                  })
            }
         } else {
            bot.sendMessage(chatId, localText.cardNumberErrorUz)
               .then(async () => {
                  await model.editStep(chatId, 'ask_card_number')
               })
         }
         ...
         if (verifyNumber) {
            ...
            if (addCardNumber) {
               bot.sendMessage(chatId, localText.askExpiredCyrillic)
                  .then(async () => {
                     await model.editStep(chatId, 'ask_expired')
                  })
            }
         } else {
            bot.sendMessage(chatId, localText.cardNumberErrorCyrillic)
               .then(async () => {
                  await model.editStep(chatId, 'ask_card_number')
               })
         }
      ...
      const isValidDateFormat = isValidDateFormat(text)
      ...
      if (isValidDateFormat) {
         ...
         if (addCardExpiry) {
            ...
            if (atmosBind?.result?.code == "OK") {
               const addCardTransId = await model.addCardTransId(chatId, atmosBind?.transaction_id)

               if (addCardTransId) {
                  if (foundUser?.lang == 'uz') {
                     bot.sendMessage(chatId, localText.askOTPUz)
                        .then(async () => {
                           await model.editStep(chatId, 'ask_otp')
                        })
                  } else if (foundUser?.lang == 'cyrillic') {
                     bot.sendMessage(chatId, localText.askOTPCyrillic)
                        .then(async () => {
                           await model.editStep(chatId, 'ask_otp')
                        })
                  }
               ...
               if (foundUser?.lang == 'uz') {
                  bot.sendMessage(chatId, localText.askExpiredErrorUz)
                     .then(async () => {
                        await model.editStep(chatId, 'ask_expired')
                     })
               } else if (foundUser?.lang == 'cyrillic') {
                  bot.sendMessage(chatId, localText.askExpiredErrorCyrillic)
                     .then(async () => {
                        await model.editStep(chatId, 'ask_expired')
                     })
               }
   ...
   if (msg.contact && foundUser?.step == 'ask_contact') {
      ...
      if (addPhoneUser) {
         if (foundUser?.lang == 'uz') {
            ...
            bot.sendMessage(chatId, localText.askCardNumberUz)
               .then(async () => {
                  await editStep(chatId, 'ask_card_number')
               })
         } else if (foundUser?.lang == 'cyrillic') {
            bot.sendMessage(chatId, localText.successfulCyrillic)
            bot.sendMessage(chatId, localText.askCardNumberCyrillic)
               .then(async () => {
                  await editStep(chatId, 'ask_card_number')
               })
         }
